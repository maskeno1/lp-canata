...
export default async (request) => {
  ...
  if (request.method === 'GET') {
    try {
      ...
      console.error('Failed to load profile signature', error);
      return json({ error: '共有ストレージから署名画像を読み込めませんでした' }, 500);
    }
  }
  ...
  if (signature.size > 3 * 1024 * 1024) {
    return json({ error: '画像サイズは3MB以下にしてください' }, 413);
  }

  try {
    await store.set(key, signature, {
      metadata: { contentType: signature.type },
    });
    return json({ status: 'ok' });
  } catch (error) {
    console.error('Failed to save profile signature', error);
